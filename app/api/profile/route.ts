...
function buildQueries(topic: string): string[] {
  const t = topic.trim();
  return [t, `${t} 是什么`, `如何看待${t}`, `${t} 为什么`, `${t} 经历`];
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!passedAnswers.length) {
      ...
      const { value } = await withCache(`search:${topic}:multi`, 60 * 60 * 1000, () =>
        searchZhihuMulti(buildQueries(topic))
      );
      corpus = value;
      ...
      if (corpus.length < 10) {
        try {
          ...
          for (const a of single) if (!seen.has(a.url || a.id)) corpus.push(a);
        } catch { /* 忽略兜底失败 */ }
      }
    } else {
    ...
    const toProfile = topByVotes(corpus, topN); // 画像聚焦 Top N 高赞
    const corpusTotal = corpus.length;

    // 画像结果缓存（key 含 topic + LLM 开关 + 语料量，避免降级/正式版混淆）
    const { value: result, cached } = await withCache(
      `profile:${topic || "adhoc"}:${llmEnabled() ? "llm" : "rule"}:${corpusTotal}:${toProfile.length}`,
      6 * 60 * 60 * 1000,
      async () => {
        const categories = await categorizeQuestions(corpus);
        const profiles = await profileAnswers(toProfile, categories);
        // 统计
