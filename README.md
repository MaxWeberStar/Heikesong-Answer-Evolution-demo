...
> ⚠️ **部署前提（重要）**：本项目的知乎数据通过 `zhihu-cli` 二进制调用（`lib/zhihu.ts` 用 `child_process`）。
> 该 CLI 由**看山工作台桌面端注入**，云端环境默认没有。上 AiWorks / 服务器部署前需二选一：
> ① 部署环境提供 `zhihu-cli` 并用环境变量 `ZHIHU_CLI` 指向它；
> ② 或将 CLI 调用替换为知乎开放平台 HTTP API（见 skill `http-api.md`）。
> 本地（看山工作台内）开发运行不受影响。
